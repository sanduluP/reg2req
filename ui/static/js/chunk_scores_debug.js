...
function render(container) {
    ...
    const isSelected = (chunk) => {
        if (typeof chunk.score === "number" && state.threshold != null) {
            return chunk.score >= state.threshold;
        }
        return Boolean(chunk.matched);
    };
    const selectedCount = chunks.filter(isSelected).length;
    ...
    const rows = chunks.map(c => {
        ...
        const scoreCell = c.score == null
            ...
            : `<span class="fw-semibold">${c.score.toFixed(3)}</span>`;
        return `
          <tr>
            <td>${c.index}</td>
            <td>${scoreCell}</td>
            <td><span class="badge ${selected ? "text-bg-success" : "text-bg-secondary"}">${selected ? "Selected" : "Dropped"}</span></td>
    ...
    container.innerHTML = `
      <div class="border rounded p-2" style="background:#fffdf5;">
        <div class="d-flex align-items-center gap-2 flex-wrap mb-2">
          ...
          <span class="small text-muted">${selectedCount}/${chunks.length} selected${state.threshold != null ? ` · threshold ${state.threshold.toFixed(2)}` : ""}</span>
        </div>
        <div style="max-height:320px; overflow:auto;">
          <table class="table table-sm table-hover align-middle mb-0" style="font-size:0.78rem;">
            <thead><tr>
              ...
              <th style="width:6rem;">Status</th><th style="width:9rem;">Match</th><th>Chunk excerpt</th>
            </tr></thead>
