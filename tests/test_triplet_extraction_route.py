...
def test_triplet_extraction_route_preserves_metadata_and_existing_policy(monkeypatch):
        ...
        return [
            {
                ...
                "schema_template": "Concept --IsA--> Concept",
                "grounding_confidence": 0.88,
                "matched_schema_nodes": ["Model"],
                "inferred_node_types": ["Operator"],
                "schema_notes": [],
            },
            {
                ...
                "schema_notes": ["No standard schema fit."],
            },
        ]
    ...
    assert captured["schema_grounding_enabled"] is True
    ...
    assert rows[0]["schema_status"] == "SCHEMA_VALID"
    assert rows[0]["matched_schema_nodes"] == ["Model"]
    ...
    assert rows[1]["schema_status"] == "NO_SCHEMA_FIT"
    # Excluded for its schema status — EXISTING alone no longer blocks
    # eligibility, because re-submitting appends overlap provenance.
    assert rows[1]["upsert_eligible"] is False


def test_existing_decision_with_valid_schema_is_upsert_eligible(monkeypatch):
    from kbdebugger.extraction import triplet_extraction_batch
    from ui.routes import pipeline_routes
    from ui.services.job_store import JOB_STORE
    from ui.ui_app.factory import create_app
    ...
    row = record.result["extracted_triplets"][0]
    assert row["decision"] == "EXISTING"
    assert row["upsert_eligible"] is True
