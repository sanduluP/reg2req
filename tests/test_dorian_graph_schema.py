...
from kbdebugger.graph.utils import (
    ...
    predicate_to_dorian_relationship_type,
...
def test_predicate_to_dorian_relationship_type_converts_allowed_predicates() -> None:
    assert predicate_to_dorian_relationship_type("HasParameter") == "has_parameter"
    assert predicate_to_dorian_relationship_type("IsA") == "is_a"
    assert predicate_to_dorian_relationship_type("SurfacesRisk") == "surfaces_risk"


def test_predicate_to_dorian_relationship_type_rejects_unknown_predicates() -> None:
    with pytest.raises(ValueError, match="Unsupported predicate"):
        predicate_to_dorian_relationship_type("invented relationship")
...
def test_graph_store_upsert_uses_node_name_and_typed_relationship() -> None:
    ...
    assert "MERGE (s)-[rel:`has_parameter` {source: $rel_source_key}]->(t)" in cypher
...
def test_graph_store_upsert_rejects_unknown_predicate_before_query() -> None:
    store = GraphStore(driver=None)  # type: ignore[arg-type]
    called = False
...
def test_retriever_reads_dorian_node_names_and_relationship_types(monkeypatch: pytest.MonkeyPatch) -> None:
    queries: list[str] = []
